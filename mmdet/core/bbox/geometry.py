...
def normalize_det(det_boxes, gt_bboxes, map_size):

def saliency_aware_bbox_overlaps(bboxes1, bboxes2, gt_saliency_maps, b=0, map_size=224):
    """Calculate overlap between two set of bboxes.
    ...
    with torch.no_grad():
        saliency_aware_intersection=bboxes1.data.new_zeros(rows,cols)
        normalized_detections=normalize_det(bboxes2, bboxes1, map_size)
        normalized_gts=
        lt = torch.max(normalized_gts[:, None, :2], normalized_detections[:, :2])  # [rows, cols, 2]
        rb = torch.min(normalized_gts[:, None, 2:], normalized_detections[:, 2:])  # [rows, cols, 2]        
        intersected_regions=torch.concat([lt,rb], axis=2)
        for i in range(rows):
            saliency_aware_overlap=integral_image_fetch(gt_saliency_maps[i], intersected_regions[i,:,:])
            area_detections=(normalized_detections[:, 2] - normalized_detections[:, 0] + 1) * (
            normalized_detections[:, 3] - normalized_detections[:, 1] + 1)
            ious = saliency_aware_overlap / ((map_size*map_size) + area2 - saliency_aware_overlap)
