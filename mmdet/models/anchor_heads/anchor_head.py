    ...
    def loss_single(self, cls_score, bbox_pred, labels, label_weights,
        ...
        with torch.no_grad():
            if idx.size()[0]>0:
                ...
                det_cls_score_max = cls_score[idx, :].max(dim=1).cpu().numpy()
                if loss_bbox.dim()>0:
                ...
                tuples=np.concatenate((np.expand_dims(anchor_iou,1),np.expand_dims(anchor_segmrate,1),np.expand_dims(det_iou,1), np.expand_dims(det_cls_score,1), np.expand_dims(det_cls_loss,1),, np.expand_dims(det_cls_score_max,1)),axis=1)
